...
export default function WaiterPage() {
  ...
  return <main className="wp-page"><div className="wp-shell"><button type="button" className="wp-adminBack" onClick={() => navigate("/admin/home")}>← Vrati na admin dashboard</button><header className="wp-top"><div><h1 className="wp-title">Dashboard osoblja</h1><div className="wp-meta">Univerzalni hotelski staff pristup</div></div><button className="wp-btn wp-btn--ghost" onClick={logout}>Odjava</button></header>{error && <div className="wp-alert wp-alert--error">{error}</div>}{loading ? <div className="wp-empty">Učitavanje…</div> : <><section className="wp-section"><h2 className="wp-h2">Pozivi</h2><div className="wp-grid">{calls.length ? calls.map((call) => <article key={call.id} className="wp-card"><div className="wp-cardInner"><div className="wp-cardTop"><div><div className="wp-cardTitle">Soba {call.tableId}</div><div className="wp-meta">{call.type === "bill" ? "Zahtjev za račun" : "Zahtjev za pomoć"}</div>{call.message && <div className="wp-callMessage" dir="auto">{call.message}</div>}</div><button disabled={busyId === call.id} onClick={() => action(`/calls/${call.id}/handle`, "PATCH", call.id)} className="wp-btn wp-btn--primary">Obrađeno</button></div></div></article>) : <div className="wp-empty">Nema otvorenih poziva.</div>}</div></section><section className="wp-section"><h2 className="wp-h2">Nove narudžbe</h2><div className="wp-grid">{orders.length ? orders.map((order) => card(order)) : <div className="wp-empty">Nema novih narudžbi.</div>}</div></section><section className="wp-section"><h2 className="wp-h2">Preuzete narudžbe</h2><div className="wp-grid">{claimedOrders.length ? claimedOrders.map((order) => card(order, true)) : <div className="wp-empty">Nema preuzetih narudžbi.</div>}</div></section></>}</div></main>;
}
